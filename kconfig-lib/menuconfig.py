...
def _load_config():
    ...
    return True if not os.path.exists(_conf_filename) else _needs_save()
...
def _value_info(sym):
    ...
    return f"""Value: {f'"{sym.str_value}"' if sym.orig_type == STRING else sym.str_value}\n"""
...
def _include_path_info(node):
    ...
    return f'Included via {" -> ".join(f"{filename}:{linenr}" for filename, linenr in node.include_path)}\n'
